...
class ProcessorsTests(unittest.TestCase):
    def test_doc_1(self):
        ...
        with open("resources/test_parsers/test_doc_1.pdf", "rb") as f:
            pdf = BytesIO(f.read())
    ...
    def test_doc_2(self):
        ...
        with open("resources/test_parsers/test_doc_2.pdf", "rb") as f:
            pdf = BytesIO(f.read())
            pdf_out = process_pdf(pdf)
            print(pdf_out)
            self.assertEqual(pdf_out, output)
    ...
    def test_doc_3(self):
        ...
        with open("resources/test_parsers/test_doc_3.pdf", "rb") as f:
            pdf = BytesIO(f.read())
            pdf_out = process_pdf(pdf)
            print(pdf_out)
            self.assertEqual(pdf_out, output)
    ...
    def test_doc_4(self):
        output = ["Example of Data Table 1 11 33 Example of Data Table 2 /97 = 75% /79 = 17% /19 = 68% /79 = 91% Example of Measuring Ocean Color Graph 1",
                  "Example of Data Table 3 9 6 9 7 7 9 9 Example of Data Table 4 Example of Measuring Ocean Color Graph 2"]
        with open("resources/test_parsers/test_doc_4.pdf", "rb") as f:
            pdf = BytesIO(f.read())
            pdf_out = process_pdf(pdf)
            print(pdf_out)
            self.assertEqual(pdf_out, output)
